...
// Calque « encre » : un masque plein couvre le parent, et le mouvement de la souris
// y creuse des taches d'encre (composite destination-out) qui révèlent ce qui est
// dessous, puis se referment en s'estompant. Canvas pur, aucune dépendance.
interface InkRevealProps {
  /** Couleur RGB du masque, ex. [252, 250, 248] */
  maskColor?: [number, number, number];
  /** Rayon de chaque tache (px) */
  brushSize?: number;
  /** Durée de vie d'une tache avant disparition (ms) */
  lifetime?: number;
  /** Rayon initial avant expansion */
  rStart?: number;
  /** Variation aléatoire du rayon (0–1) */
  rVary?: number;
  /** Distance min entre deux taches le long d'un trait (px) */
  stampStep?: number;
  /** Nombre max de taches simultanées (les plus vieilles sont retirées) */
  maxStamps?: number;
  /** Segments du cercle ondulé (plus = plus lisse) */
  segments?: number;
  /** Amplitudes d'ondulation [primaire, secondaire, tertiaire] */
  wobble?: [number, number, number];
  /** Rayon interne du dégradé (0–1, relatif au rayon de la tache) */
  gradientInnerRadius?: number;
  /** Opacités du dégradé [centre, milieu, bord] */
